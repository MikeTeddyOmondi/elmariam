<script lang="ts">
  ...
  import { Card, CardContent, CardHeader, CardTitle, Chart } from '@elmariam/ui';
  import { Chart as ChartJS, registerables } from 'chart.js';
  import { BedDouble, Receipt, GlassWater, ClipboardList, TrendingUp } from 'lucide-svelte';
  ...
  getDashboardStats()
    .then((d: any) => { statsData = d; loading = false; })
    .catch((e: any) => { statsError = e.message; loading = false; });
    ...
    activityChart = new ChartJS(activityCanvas, {
      ...
      options: {
        ...
        scales: {
          ...
          y: { grid: { color: 'rgba(255,255,255,0.05)' }, ticks: { color: cssVar('--muted-foreground') } },
        },
      },
    });
    ...
    paymentChart = new ChartJS(paymentCanvas, {
      ...
      data: {
        ...
        datasets: [{
          ...
          backgroundColor: entries.map(([k]) => (paymentConfig as any)[k]?.color ?? cssVar('--chart-5')),
          borderWidth: 0,
          hoverOffset: 8,
        }],
      },
      options: {
        ...
        plugins: {
          legend: {
            position: 'bottom',
            labels: { color: cssVar('--muted-foreground'), padding: 16, boxWidth: 12 },
          },
        },
      },
    });
    ...
    revenueChart = new ChartJS(revenueCanvas, {
      ...
      data: {
        ...
        datasets: [{
          label: 'Revenue (KES)',
          data: [
            statsData.invoices.reduce((s: number, i: any) => s + (i.totalCost || 0), 0),
            statsData.sales.reduce((s: number, s2: any) => s + (s2.totalStockValue || 0), 0),
          ],
...
<div class="space-y-6">
  ...
  {#if loading}
    <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
      {#each Array(4) as _}
        <Card>
          <CardHeader class="pb-2"><div class="h-4 w-24 bg-secondary animate-pulse rounded"></div></CardHeader>
          <CardContent><div class="h-8 w-12 bg-secondary animate-pulse rounded"></div></CardContent>
        </Card>
      {/each}
    </div>
    <div class="grid gap-6 lg:grid-cols-3">
      {#each [2,1] as span}
        <Card class={span === 2 ? 'lg:col-span-2' : ''}>
          <CardContent class="pt-6"><div class="h-64 bg-secondary animate-pulse rounded"></div></CardContent>
        </Card>
      {/each}
    </div>
  ...
  {:else if statsData}
        ...
        <Card>
          ...
          <CardContent>
            <div class="text-3xl font-bold text-foreground">
              {Array.isArray(statsData[def.key]) ? statsData[def.key].length : 0}
            </div>
          </CardContent>
        </Card>
      {/each}
    ...
    <div class="grid gap-6 lg:grid-cols-3">
      <Card class="lg:col-span-2">
        <CardHeader>
          <CardTitle class="text-base font-semibold text-foreground">Activity Overview</CardTitle>
          <p class="text-xs text-muted-foreground">Total records per category</p>
        </CardHeader>
        <CardContent>
          <Chart.Container config={activityConfig} class="min-h-[240px] w-full">
            <canvas bind:this={activityCanvas} class="w-full" style="height:240px"></canvas>
          </Chart.Container>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          <CardTitle class="text-base font-semibold text-foreground">Payment Methods</CardTitle>
        ...
        <CardContent>
          <Chart.Container config={paymentConfig} class="min-h-[240px] w-full">
            <canvas bind:this={paymentCanvas} class="w-full" style="height:240px"></canvas>
          </Chart.Container>
        </CardContent>
      </Card>
    </div>
    ...
    <div class="grid gap-6 lg:grid-cols-3">
      <Card>
        <CardHeader>
          <div class="flex items-center gap-2">
            ...
            <CardTitle class="text-base font-semibold text-foreground">Revenue (KES)</CardTitle>
          </div>
          <p class="text-xs text-muted-foreground">Hotel & bar earnings total</p>
        </CardHeader>
        <CardContent>
          <Chart.Container config={revenueConfig} class="min-h-[200px] w-full">
            <canvas bind:this={revenueCanvas} class="w-full" style="height:200px"></canvas>
          </Chart.Container>
        </CardContent>
      </Card>
      ...
      <Card class="lg:col-span-2">
        <CardHeader>
          <CardTitle class="text-base font-semibold text-foreground">Recent Bookings</CardTitle>
            ...
            <thead class="border-b border-border">
              <tr>
                {#each ['Guest','Room Type','Check-in','Check-out','Payment'] as h}
                  <th class="text-left px-4 py-2.5 text-xs font-medium text-muted-foreground uppercase">{h}</th>
                {/each}
              </tr>
            </thead>
            <tbody>
              {#each statsData.bookings.slice(-5).reverse() as b}
                <tr class="border-b border-border last:border-0 hover:bg-secondary/30 transition-colors">
                  <td class="px-4 py-2.5 text-foreground">
                    {b.customer?.firstname ?? b.occupant?.firstname ?? '—'} {b.customer?.lastname ?? b.occupant?.lastname ?? ''}
                  </td>
                  <td class="px-4 py-2.5 text-muted-foreground capitalize">{b.roomType?.roomType ?? b['room-type']?.roomType ?? '—'}</td>
                  <td class="px-4 py-2.5 text-muted-foreground">{b.checkInDate ? new Date(b.checkInDate).toLocaleDateString() : '—'}</td>
                  <td class="px-4 py-2.5 text-muted-foreground">{b.checkOutDate ? new Date(b.checkOutDate).toLocaleDateString() : '—'}</td>
                  <td class="px-4 py-2.5">
                    <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-secondary text-muted-foreground capitalize">
                      {b.invoice?.paymentMethod ?? b.invoiceRef?.paymentMethod ?? '—'}
                    </span>
                  </td>
                </tr>
              {:else}
                <tr><td colspan="5" class="px-4 py-6 text-center text-sm text-muted-foreground">No bookings yet</td></tr>
              {/each}
            </tbody>
          </table>
        </CardContent>
